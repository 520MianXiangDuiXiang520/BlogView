  ...
  <div>
    <div class="title">
      <el-row :gutter="20">
        ...
        <el-col :span="4">
          <el-button type="success" size="medium" class="button" @click="getTags()">提交</el-button>
        </el-col>
      </el-row>
    </div>

    <el-dialog title="发布文章" :visible.sync="centerDialogVisible" width="50%" center>
      <div class="choiseTag">
        <el-row :gutter="20">
          <el-col :span="4">
            <span>文章标签</span>
          ...
          <el-col :span="20">
            <el-row :gutter="20">
              <el-col :span="4" v-for="(tag, index) in choiseTags" :key="'has' + tag.id">
                <div class="grid-content bg-purple tag" @click="deleteTag(index)">
      ...
      <div class="choiseTag">
        <el-row :gutter="20">
          ...
          <el-col :span="20">
            <el-row :gutter="20">
              <el-col :span="4" v-for="(tag, index) in tags" :key="'choise' + index">
                <div @click="choiseTag(index)" class="grid-content bg-purple tag">
                  <el-tag>{{tag.name}}</el-tag>
                </div>
              </el-col>
            </el-row>
      ...
      <div class="newTag">
        <el-row :gutter="20">
          ...
          <el-col :span="16">
            <el-input placeholder="请输入标签名" v-model="newTag" clearable></el-input>
          </el-col>
    ...
    getDetail() {
      ...
      this.axios({
        ...
        self.title = response.data["title"];
          ...
          self.openMessage("[ "+ error.response.status +" ] 获取文章详情失败！", "warning")
        });
    },
    // 获取本文 tags
    getArticleTags() {
      ...
      this.axios({
        method: "post",
        url: "http://localhost:8080/api/article/tags",
        data: {
          articleID: parseInt(self.$route.params.id),
        },
      }).then(function (response) {
        ...
        for (let i = 0; i < response.data["tags"].length; i++) {
          ...
          self.openMessage("[ "+ error.response.status +" ] 获取文章标签失败！", "warning")
        });
    },
    // 提交更新
    release() {
      ...
      this.axios({
        method: "post",
        url: "http://localhost:8080/api/article/update",
        data: {
          title: this.title,
          text: this.handbook,
    ...
    getTags() {
      ...
      this.axios({
        ...
        for (let i = 0; i < self.tags.length; i++) {
          ...
          self.openMessage("[ "+ error.response.status +" ] 获取标签列表失败！", "warning")
        });
    },
    choiseTag(index) {
